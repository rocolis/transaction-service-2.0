...
def remove_undefined(text):
    try:
        return text.replace("undefined", "").replace("--", "-").strip("-")
    except:
        pass
...
def remove_dates(text):
    try:
        return re.sub(r'\d{4}-\d{2}-\d{2}', '', text).strip("-")
    except:
        pass
...
def send_email_verification(otp_code, user_mail, subject, html_file_path, username=None, ad_id=None):
    ...
    context = {'validation_code': otp_code, 'username': username,
               'ad_id': ad_id}  # The value of variable in html {{ name }}
    receiver = user_mail
    subject = subject
...
def add_otp_code(user, html_file_path, users_collection):
    try:
        if not 'attempts' in user or user['attempts'] < 5:
            ...
            if users_collection.update_one({"_id": ObjectId(user['_id'])}, {
                "$set": {"otp_code": otp_code, "attempts": update_attempts,
                         "otp_code_expiration": datetime.datetime.now() + datetime.timedelta(minutes=15)}}):
                send_email_verification(otp_code, user['email'], "ROCOLIS - CODE DE VERIFICATION", html_file_path,
                                        user['firstName'])
...
def compare_travel_and_parcel(travel_collection, parcel_collection, travel_id, parcel_id):
    try:
        ...
        if travel and parcel:
            if travel["departureCity"] == parcel["departureCity"] and travel["destinationCity"] == parcel[
                "destinationCity"] and travel["departureCountry"] == parcel["departureCountry"] and travel[
                "destinationCountry"] == parcel["destinationCountry"]:
                if travel["availableKilos"] >= parcel["availableKilos"]:
                    pass
                else:
                    return jsonify(
                        {"response": False, "message": "Le nombre de kilos disponibles est insuffisant"}), 400
            else:
                return jsonify({"response": False,
                                "message": "La ville de départ ou d'arrivée présente sur le colis, ne correspond pas à celui du voyageur"}), 400
        else:
...
def update_acces_refresh_tokens(sub, access_token, refresh_token, users_collection):
    try:
        users_collection.update_one({"_id": ObjectId(sub)},
...
def get_traveler_details(user_id, users_collection):
    try:
        ...
        return {'created': user['created'], 'name': user['name'], 'firstName': user['firstName'],
